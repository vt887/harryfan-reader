...
final class PrintManagerQuickSpec: QuickSpec {
    // Main test entry point for PrintManager pagination tests
    override class func spec() {
        describe("PrintManager pagination") {
            it("returns 1 page for very short content") {
                Self.testSinglePageForShortContent()
            }
            it("computes expected pages for many short lines") {
                Self.testPagesForManyShortLines()
            }
        }
    }

    // Tests that a very short string fits on a single page
    private static func testSinglePageForShortContent() {
        let pm = PrintManager()
    ...
    // Tests that the number of pages for many short lines matches the expected calculation (with tolerance)
    private static func testPagesForManyShortLines() {
        let pm = PrintManager()
        let font = NSFont.monospacedSystemFont(ofSize: 10.0, weight: .regular)
